//
//  GZCERepresentHomeViewController.m
//  GZMedicalHealth
//
...
@interface GZCERepresentHomeViewController ()<BMKLocationAuthDelegate,BMKLocationManagerDelegate>
{
    NSString *_lng;
    NSString *_lat;
    NSString *_city;
...
@implementation GZCERepresentHomeViewController
...
- (void)nicknameData
{
    ...
    [[CMHHTTPRequest requestWithParameters:paramters] enqueueResultClass:GZCERepresentBaseModel.class parsedResult:YES isRequestHead:NO isShowHudMsg:NO  success:^(NSURLSessionDataTask *task, GZCERepresentBaseModel *responseObject) {
        ...
        [defaults setObject:responseObject.status forKey:@"jinStatus"];
        [defaults synchronize];
        
        self.nickNameLab.text = [NSString stringWithFormat:@"你好，%@",responseObject.nickname];
        [self.jifenBtn setTitle:[NSString stringWithFormat:@"%.2f",responseObject.jifen] forState:UIControlStateNormal];
...
- (IBAction)clickEvents:(UIButton *)sender
{
    switch (sender.tag) {
        case 10:    // 终端绩效
        {
            ...
            [self.navigationController pushViewController:[GZMonthlyAchievementViewController new] animated:YES];
        }
            break;
