...
typedef NS_ENUM(NSUInteger, CMHHTTPResponseCode) {
    CMHHTTPResponseCodeSuccess  = 1 ,                     /// 请求成功    (PS：根据自身项目去设置)
    CMHHTTPResponseCodeNotLogin = 2 ,                     /// 用户尚未登录,或者Token失效 (PS：根据自身项目去设置)
    CMHHTTPResponseCodeNotData = 100 ,                     /// 列表数据为空，（空字典，空数组）
};
