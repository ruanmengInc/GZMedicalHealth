...
/** 扫描内容的 高度 */
#define scanBorderH 0.6 * self.frame.size.height
...
- (void)initialization {
    ...
    [self.layer addSublayer:self.tipsLabel.layer];
    [self.layer addSublayer:self.tipsTwoLabel.layer];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    // 设置空白区
    UIBezierPath *bezierPath = [UIBezierPath bezierPathWithRect:CGRectMake(scanBorderX, scanBorderY, scanBorderW, scanBorderH)];
    [bezierPath fill];
    // 执行混合模式
...
- (UIImageView *)scanningline {
    if (!_scanningline) {
        ...
        _scanningline.backgroundColor = [UIColor redColor];
        _scanningline.frame = CGRectMake(scanBorderX, scanBorderY + 0.5 * scanBorderH, scanBorderW, 2);
    }
...
{
    if (_tipsLabel == nil) {
        _tipsLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, self.contentView.GZ_bottom + 12, self.GZ_width, 20)];
        _tipsLabel.text = @"将药品二维码/条形码放入框内";
...
{
    if (_tipsTwoLabel == nil) {
        _tipsTwoLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, self.tipsLabel.GZ_bottom + 5, self.GZ_width, 20)];
        _tipsTwoLabel.text = @"即可自动扫描";
        _tipsTwoLabel.font = [UIFont systemFontOfSize:15];
        _tipsTwoLabel.textColor = [UIColor whiteColor];
        _tipsTwoLabel.textAlignment = NSTextAlignmentCenter;
    }
