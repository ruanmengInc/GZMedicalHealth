...
@implementation CLLocation (GZLocation)
...
// b = a * (1 - f)
// ee = (a^2 - b^2) / a^2;
const double GZa = 6378245.0;
const double GZee = 0.00669342162296594323;
...
void transform_earth_from_mars(double lat, double lng, double* tarLat, double* tarLng)
{
    ...
    magic = 1 - GZee * magic * magic;
    double sqrtMagic = sqrt(magic);
    dLat = (dLat * 180.0) / ((GZa * (1 - GZee)) / (magic * sqrtMagic) * M_PI);
    dLon = (dLon * 180.0) / (GZa / sqrtMagic * cos(radLat) * M_PI);
    *tarLat = lat + dLat;
    *tarLng = lng + dLon;
}
