...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [self.navigationController.navigationBar setBarTintColor:UIColorHex(0x4ab3ff)];
}
...
-(void)viewWillDisappear:(BOOL)animated
{
    ...
    [self.navigationController.navigationBar setBarTintColor: MHColorFromHexString(@"#FFFFFF")];
...
- (void)configure{
    ...
    example07.operation = ^{
        ...
        vc.callback = ^(NSString *name) {
            @strongify(self);
            self.name = name;
            [MBProgressHUD SG_showMBProgressHUDWithOnlyMessage:[NSString stringWithFormat:@"你的名字叫 -- %@",name] delayTime:1.0];
        };
