...
+ (void) _setupNavigationBarTheme{
    ...
    // 设置导航栏的背景渲染色
    CGFloat rgb = 1;
    [appearance setBarTintColor:[UIColor colorWithRed:rgb green:rgb blue:rgb alpha:1]];
    // 设置导航栏的背景渲染色或背景图片
//    [appearance setBackgroundImage:[UIImage imageNamed:@"NavBackGround"] forBarMetrics:UIBarMetricsDefault];
    ...
    textAttrs[NSForegroundColorAttributeName] = MHColorFromHexString(@"#343434");
...
+ (void)_setupBarButtonItemTheme{
    ...
    // 设置普通状态的文字属性
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
    textAttrs[NSForegroundColorAttributeName] = MHColorFromHexString(@"#010101");
    ...
    highTextAttrs[NSForegroundColorAttributeName] = [MHColorFromHexString(@"#010101") colorWithAlphaComponent:.5f];
...
- (void)showNavigationBottomLine
{
    ...
    if (self.lineColor == nil) {
        ...
        self.navigationBottomLine.backgroundColor = MHColorFromHexString(@"#e9e9e9");
        
    }else
    {
        self.navigationBottomLine.backgroundColor = self.lineColor;
    }
}
...
//        }
        ...
        if (MHStringIsNotEmpty(self.titleName)) {

            if (self.titleNameColor != nil) {
                
                viewController.navigationItem.leftBarButtonItem = [UIBarButtonItem mh_backItemWithTitle:self.titleName selectTitle:nil titleColor:self.titleNameColor titleFont:MHRegularFont_17 imageName:@"barbuttoniconWhiteBack" target:self action:@selector(_backItemDidClicked)];

            }else
            {
                viewController.navigationItem.leftBarButtonItem = [UIBarButtonItem mh_backItemWithTitle:self.titleName selectTitle:nil titleColor:MHColorFromHexString(@"#343434") titleFont:MHRegularFont_17 imageName:@"navigationButtonReturn" target:self action:@selector(_backItemDidClicked)];
            }
            
        } else
        {
            viewController.navigationItem.leftBarButtonItem = [UIBarButtonItem mh_backItemWithTitle:@"" selectTitle:nil titleColor:MHColorFromHexString(@"#343434") titleFont:MHRegularFont_17 imageName:@"barbuttoniconWhiteBack" target:self action:@selector(_backItemDidClicked)];
        }
