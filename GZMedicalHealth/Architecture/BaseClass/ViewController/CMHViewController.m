...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if ([nav isKindOfClass:[CMHNavigationController class]]) { /// 容错
        ...
        nav.lineColor = self.prefersNavigationBarBottomLineColor;
...
-(void)ShowAlertWithMessage:(NSString *)message{
    ...
    //修改按钮的颜
    [sure setValue:MHColorFromHexString(@"#3a3a3a") forKey:@"_titleTextColor"];
...
-(void)ShowAlertTitle:(NSString *)title Message:(NSString *)message Delegate:(id)delegate  cancelButtonTitle:(NSString *)cancel otherButtonTitles:(NSString *)other Block:(AlertBlock)block{
    ...
    if (MHStringIsEmpty(title)) {
        title = @"提示";
    }
    
    if (MHStringIsEmpty(cancel)) {
        cancel = @"取消";
    }
    
    if (MHStringIsEmpty(other)) {
        other = @"确定";
    }
    ...
    //修改按钮的颜
    [sure setValue:MHColorFromHexString(@"#3a3a3a") forKey:@"_titleTextColor"];
    [cancle setValue:MHColorFromHexString(@"#3a3a3a") forKey:@"_titleTextColor"];
